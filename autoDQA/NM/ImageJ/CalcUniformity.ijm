...
var roiStd 	= 0;
	...
	mtxSizeX = getWidth();
	mtxSizeY = getHeight();
	getPixelSize(unit, pixSizeX, pixSizeY);
	pixSize = pixSizeX;
	...
	//dummy 	= printResultsToFile(0);		//print the results to file

	//dummy 	= writeJPEG(0);				//generate a nice summary screenshot
...
function calcParams(roiIndex){
	...
	roiIU = (maxVal-minVal)/(maxVal+minVal);
